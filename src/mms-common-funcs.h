...
static void interp_asf_header_case_bitrate(TYPE *this, int i)
{
...
static void interp_asf_header_case_header_extenstion(TYPE *this, int *guid, uint64_t length, int i)
{
  if ((24 + 18 + 4) > length)
    return;
  ...
  while ((i + 24) <= this->asf_header_len) {
    ...
    switch (guid) {
      ...
      case GUID_ASF_STREAM_BITRATE_PROPERTIES:
        interp_asf_header_case_bitrate(this, i);
        break;

      case GUID_ASF_HEADER_EXTENSION:
        interp_asf_header_case_header_extenstion(this, &guid, length, i);
        break;
...
static void mms_get_best_stream_audio(TYPE *this, int *audio_stream, int *audio_rate)
{
  int     i;
...
static void mms_get_best_stream_video(TYPE *this, int *bandwidth_left, int *min_bw_left, int *video_stream, int *video_rate)
{
  int i;
  for (i = 0; i < this->num_stream_ids; i++) {
    switch (this->streams[i].stream_type) {
  ...
  int *video_stream_ret)
{
  ...
  mms_get_best_stream_audio(this, &audio_stream, &audio_rate);
  ...
  mms_get_best_stream_video(this,&bandwidth_left,&min_bw_left,&video_stream,&video_rate);
