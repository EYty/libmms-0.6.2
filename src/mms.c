...
#ifdef INCLUDE_LMF
static int enable_libmms_read=1;
#define MMS_CONNECT_TIMEOUT 12
#define MMS_READ_TIMEOUT 6
static int connect_with_timeout(int sockfd, const struct sockaddr *addr, socklen_t addrlen, time_t timeout_sec);
static ssize_t read_with_timeout(int fd, void *buf, size_t count, time_t timeout_sec);
#endif
...
/*   lprintf("%d\n", fallback_io_select(data, socket, MMS_IO_READ_READY, 1000)); */
  ...
  {
#ifdef INCLUDE_LMF
    if( !enable_libmms_read) {
      lprintf("%s:%d:read disabled!\n", __FUNCTION__, __LINE__);
      break;
    }
    ret = (off_t)read_with_timeout(socket, buf + len, num - len, MMS_READ_TIMEOUT);
#else
#ifdef WIN32
...
static int fallback_io_tcp_connect(void *data, const char *host, int port)
{
  ...
  for (r = res; r != NULL; r = r->ai_next) {
    ...
    if (s != -1) {
#ifdef INCLUDE_LMF
      if (connect_with_timeout(s, r->ai_addr, r->ai_addrlen, MMS_CONNECT_TIMEOUT) != -1) {
#else
...
static int get_packet_header (mms_io_t *io, mms_t *this, mms_packet_header_t *header) {
  ...
  if (LE_32(this->buf + 4) == MMS_SESSION_ID_FIELD_VALUE) { // Clarified kevin.jung
    /* command packet */
...
static int get_packet_command (mms_io_t *io, mms_t *this, uint32_t packet_len) {
  ...
  if (LE_32(this->buf + 12) != MMS_SEAL_FIELD_VALUE) { // Clarified kevin.jung
...
static int get_answer (mms_io_t *io, mms_t *this) {
  ...
  switch (get_packet_header (io, this, &header)) {
    ...
    case MMS_PACKET_COMMAND:
      ...
      if (command == MMSCMD_LINKMACTOVIEWER_PING) { // Clarified kevin.jung
        if (!send_command (io, this, MMSCMD_LINKVIEWERTOMAC_PONG, 0, 0, 0)) { // Clarified kevin.jung
  ...
  while (!stop) {
    ...
    switch (get_packet_header (io, this, &header)) {
      ...
      case MMS_PACKET_ASF_PACKET:
        ...
        if ((header.flags == MMS_ASFHEADER_STOP_AFFLAG_VALUE_8) ||
			(header.flags == MMS_ASFHEADER_STOP_AFFLAG_VALUE_C)) // Clarified kevin.jung
          stop = 1;
        break;
    }
  }
...
int static mms_choose_best_streams(mms_io_t *io, mms_t *this) {
  ...
  lprintf("send command 0x33\n"); // Clarified kevin.jung
  if (!send_command (io, this, MMSCMD_LINKVIEWERTOMAC_STREAMSWITCH, this->num_stream_ids,
  ...
  if ((res = get_answer (io, this)) != MMSCMD_LINKMACTOVIEWER_REPORTSTREAMSWITCH) {// Clarified kevin.jung
...
int mms_connect_requestconnect (mms_io_t *io, mms_t *this, mms_buffer_t *command_buffer,iconv_t *url_conv) {
  int     res;
  lprintf("send command 0x01\n");
  mms_buffer_init(command_buffer, this->scmd_body);
  mms_buffer_put_32 (command_buffer, MMS_VIEWERTOMAC_PROTOCOLREVISION); // Clarified kevin.jung
  ...
  if(!res)
    return -1;
  // Clarified kevin.jung
  if (!send_command(io, this, MMSCMD_LINKVIEWERTOMAC_CONNECT, 0, MMS_MACTOVIEWER_PROTOCOLREVISION, command_buffer->pos + res)) {
...
int mms_connect_connectfunnel (mms_io_t *io, mms_t *this, mms_buffer_t *command_buffer,iconv_t *url_conv) {
  int res;
  mms_buffer_put_32 (command_buffer, 0x00000000);
  mms_buffer_put_32 (command_buffer, MMS_PREDEFINED_MAXBITRATE); // Clarified kevin.jung
  ...
  if(!res)
    return -1;
  // Clarified kevin.jung
  if (!send_command(io, this, MMSCMD_LINKVIEWERTOMAC_CONNECTFUNNEL, 0, MMS_PREDEFINED_MAXBLOCKBYTES, command_buffer->pos + res)) {
    lprintf("failed to send command 0x02\n");
    return -1;
  }
  switch (res = get_answer (io, this)) {
    case MMSCMD_LINKMACTOVIEWER_REPORTCONNECTEDFUNNEL: // Clarified kevin.jung
      /* protocol accepted */
      break;
    case MMSCMD_LINKMACTOVIEWER_REPORTDISCONNECTEDFUNNEL: // Clarified kevin.jung
...
int mms_connect_openfile (mms_io_t *io, mms_t *this, uint32_t *openid,iconv_t *url_conv) {
  mms_buffer_t command_buffer;
  int res;
  ...
  if(!res)
    return -1;
  // Clarified kevin.jung
  if (!send_command(io, this, MMSCMD_LINKVIEWERTOMAC_OPENFILE, 1, 0, command_buffer.pos + res)) {
    lprintf("failed to send command 0x05\n");
    return -1;
  }
  switch (res = get_answer (io, this)) {
    case MMSCMD_LINKMACTOVIEWER_REPORTOPENFILE:  // Clarified kevin.jung
      {
        int xx, yy;
        ...
        *openid = LE_32(this->buf + 48);
        ...
        xx = this->buf[62];
        yy = this->buf[63];
        this->live_flag = ((xx == 0) && ((yy & MMS_REPORTOPENFILE_FILEATTR_LIVEFLAG_MASK) == 2)); // Clarified kevin.jung
        this->seekable = !this->live_flag;
        lprintf("openid=%d, live: live_flag=%d, xx=%d, yy=%d\n", *openid, this->live_flag, xx, yy);
      }
      break;
    case MMSCMD_LINKMACTOVIEWER_SECURITYCHALLENGE: // Clarified kevin.jung
...
int mms_connect_readblock (mms_io_t *io, mms_t *this, uint32_t *openid,iconv_t *url_conv) {
  int res;
  mms_buffer_t command_buffer;
  mms_buffer_init(&command_buffer, this->scmd_body);
  mms_buffer_put_32 (&command_buffer, 0x00000000);                  /* ?? */
  ...
  mms_buffer_put_32 (&command_buffer, 0x00000000);                  /* ?? */
  if (!send_command (io, this, MMSCMD_LINKVIEWERTOMAC_READBLOCK, *openid, 0, command_buffer.pos)) {// Clarified kevin.jung
...
int mms_connect_startplay (mms_io_t *io, mms_t *this, uint32_t *openid,iconv_t *url_conv) {
  mms_buffer_t command_buffer;
  mms_buffer_init(&command_buffer, this->scmd_body);
  ...
  // FYI. In the spec, padding value should be 0x0, but here set 0x0001FFFF.
  // In the spec, additional info mentioned about arbitrary padding value can be inserted by the platform.
  // So, Server will ignore the value, 0x0001FFFF. - kevin.jung
  if (!send_command (io, this, MMSCMD_LINKVIEWERTOMAC_STARTPLAYING, 1, 0x0001FFFF, command_buffer.pos)) {// Clarified kevin.jung
...
mms_t *mms_connect (mms_io_t *io, void *data, const char *url, int bandwidth) {
  ...
  /* command 0x1 */
  if (mms_connect_requestconnect(io,this,&command_buffer, &url_conv)<0)
    goto fail;
  ...
  mms_buffer_init(&command_buffer, this->scmd_body);
  if (mms_connect_connectfunnel(io,this,&command_buffer, &url_conv)<0)
    goto fail;

  /* command 0x5 */
  if (mms_connect_openfile(io,this,&openid, &url_conv)<0)
    goto fail;

  /* command 0x15 */
  lprintf("send command 0x15\n");
  if (mms_connect_readblock(io,this,&openid, &url_conv)<0)
    goto fail;
  ...
  if (mms_connect_startplay(io,this,&openid, &url_conv)<0)
      goto fail;
...
static int get_media_packet_command (mms_io_t *io, mms_t *this, mms_packet_header_t *header)
{
  int command;
  command = get_packet_command (io, this, header->packet_len);
  ...
  switch (command) {
    ...
    case MMSCMD_LINKMACTOVIEWER_REPORTENDOFSTREAM:// Clarified kevin.jung
      {
        uint32_t error_code;
    ...
    case MMSCMD_LINKMACTOVIEWER_REPORTSTREAMCHANGE:// Clarified kevin.jung
      {
        lprintf("new stream.\n");
        /* asf header */
        if (!get_asf_header (io, this)) {
    ...
    case MMSCMD_LINKMACTOVIEWER_PING:// Clarified kevin.jung
      {
        if (!send_command (io, this, MMSCMD_LINKVIEWERTOMAC_PONG, 0, 0, 0)) {// Clarified kevin.jung
          lprintf("error sending ping reply\n");
          return 0;
    ...
    case MMSCMD_LINKMACTOVIEWER_REPORTSTARTEDPLAYING:// Clarified kevin.jung
      break;
...
static int get_media_packet (mms_io_t *io, mms_t *this) {
  ...
  switch (get_packet_header (io, this, &header)) {
    ...
    case MMS_PACKET_COMMAND:
      get_media_packet_command(io, this, &header);
      break;
...
static int mms_request_data_packet (mms_io_t *io, mms_t *this,
  ...
  /* command 0x07 */
  {
    ...
    mms_buffer_put_32 (&command_buffer, this->packet_id_type);    /* asf media packet id type */
    if (!send_command (io, this, MMSCMD_LINKVIEWERTOMAC_STARTPLAYING, 1, 0x0001FFFF, 8+command_buffer.pos)) {// Clarified kevin.jung
      lprintf("failed to send command 0x07\n");
      return 0;
    }
  }
...
static int connect_with_timeout(int sockfd, const struct sockaddr *addr, socklen_t addrlen, time_t timeout_sec)
{
  int status, flags, error = 0;
  socklen_t len = sizeof(int);
  fd_set wset;
  struct timeval tval;

  lprintf("%s:%d:socket(%d)\n", __FUNCTION__, __LINE__, sockfd);

  if( (flags = fcntl(sockfd, F_GETFL, 0)) == -1)
  {
    lprintf("%s:%d:error(%d):%s\n", __FUNCTION__, __LINE__, errno, strerror(errno));
    return -1;
  }
  ...
  lprintf("%s:%d:wait(timeout=%d sec)\n", __FUNCTION__, __LINE__, timeout_sec);
  ...
  status = select(sockfd + 1, NULL, &wset, NULL, &tval);
...
static ssize_t read_with_timeout(int fd, void *buf, size_t count, time_t timeout_sec)
{
  ...
  status = select(fd + 1, &rset, NULL, NULL, &tval);
...
int mms_enable_read (mms_t *this) {

  enable_libmms_read = 1;
  lprintf("enable_libmms_read ==> true\n");

  return 1;
}

int mms_disable_read (mms_t *this) {

  enable_libmms_read = 0;
  lprintf("enable_libmms_read ==> false\n");
