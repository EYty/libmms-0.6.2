...
#define USERAGENT "User-Agent: NSPlayer/4.1.0.3856 (compatible; LG NetCast.TV-2013)\r\n"
...
#define CHUNK_TYPE_DATA2       0x44a4	/* another data chunk type */
#define CHUNK_TYPE_END         0x4524
#define CHUNK_TYPE_ASF_HEADER  0x4824
#define CHUNK_TYPE_META        0x4d24	/* meta data stuff (playlist...) */
...
static const char* mmsh_FirstRequest =
    ...
    "Host: %s:%d\r\n"
#ifdef INCLUDE_LMF
    "Pragma: no-cache,rate=%.3f"
#else
    "Pragma: no-cache,rate=1.000000"
#endif
    ",stream-time=0,stream-offset=0:0,request-context=%u,max-duration=0\r\n"
    CLIENTGUID
    "Connection: Close\r\n\r\n";
...
static const char* mmsh_SeekableRequest =
    ...
    "Pragma: no-cache,rate=1.000000"
#endif
    ",stream-time=%u,stream-offset=%u:%u,request-context=%u,max-duration=%u\r\n"
    CLIENTGUID
    "Pragma: xPlayStrm=1\r\n"
...
//  lprintf("[chunk_header]:%08X\n", *(uint32_t *)chunk_header);
  ...
  switch (this->chunk_type) {
    ...
    case CHUNK_TYPE_DATA2: /* handle same as data	*/
      ext_header_len = 8;
      break;
    case CHUNK_TYPE_END:
      ext_header_len = 4;
    ...
    case CHUNK_TYPE_META:
      ext_header_len = 8;	/* meta has 8 bytes	*/
      break;
    default:
      ext_header_len = 0;
  }
...
//  if (ext_header_len == 8) lprintf("[ext_header]:%08X %08X\n", *(uint32_t *)&ext_header[4], *(uint32_t *)&ext_header[0]);
//  if (ext_header_len == 4) lprintf("[ext_header]:%08X\n", *(uint32_t *)ext_header);

  if (this->chunk_type == CHUNK_TYPE_DATA || this->chunk_type == CHUNK_TYPE_DATA2 || this->chunk_type == CHUNK_TYPE_END)
...
static int skip_metadata(mms_io_t *io, mmsh_t *mmsh)
{
  char *buf;
  unsigned int len = mmsh->chunk_length;
  unsigned int nr = 0;

  if (mmsh->chunk_type != CHUNK_TYPE_META)
    return 0;

  buf = calloc(1, len);
  if (!buf)
  {
    lprintf("not enought memory\n");
    return 0;
  }

  nr = io_read(io, mmsh->s, buf, len);
  if (nr != len)
    lprintf("warning: imcomplete reading for metadata\n");
    /* nothing to do. just see what's wrong here	*/
...
static int get_header (mms_io_t *io, mmsh_t *this) {
  ...
  if (this->chunk_type == CHUNK_TYPE_DATA || this->chunk_type == CHUNK_TYPE_DATA2)
  {
...
#ifdef INCLUDE_LMF
void mmsh_set_play_rate(mmsh_t *mmsh, double rate)
{
  mmsh->mmsh_play_rate = rate;
}
#endif

static int mmsh_connect_int_sub(mmsh_t *this,int *offset, int *audio_stream, int *video_stream, char* stream_selection, int ss_size)
{
  int i;
  ...
  for (i = 0; i < this->num_stream_ids; i++) {
    ...
    if ((this->streams[i].stream_id == *audio_stream) ||
      ...
      size = snprintf(stream_selection + *offset, ss_size - *offset,
...
static int mmsh_connect_int (mms_io_t *io, mmsh_t *this, off_t seek, uint32_t time_seek) {
  ...
  snprintf (this->str, SCRATCH_SIZE,
            mmsh_FirstRequest,
            this->uri,
            this->http_host, this->http_port,
#ifdef INCLUDE_LMF
            this->mmsh_play_rate,
#endif
            this->http_request_number++);
  ...
  /* 2 means disabled */
  offset = 0;
  if (mmsh_connect_int_sub(this, &offset, &audio_stream, &video_stream, stream_selection,sizeof(stream_selection))<0) {
    goto fail;
  }
...
void mmsh_connect_sub (mmsh_t *this, GURI *uri, GURI *proxy_uri) {
  if (this->proxy_url) {
...
mmsh_t *mmsh_connect (mms_io_t *io, void *data, const char *url, int bandwidth) {
  ...
  mmsh_connect_sub(this,uri,proxy_uri);
...
off_t mmsh_seek_sub (mms_io_t *io, mmsh_t *this, off_t *dest, uint32_t orig_asf_header_len, uint32_t orig_asf_packet_len)
{
  if (this->chunk_seq_number > 0) {
...
off_t mmsh_seek (mms_io_t *io, mmsh_t *this, off_t offset, int origin) {
  ...
  if (dest_packet_seq < 0) {
    return mmsh_seek_sub(io, this, &dest, orig_asf_header_len, orig_asf_packet_len);
  }
...
off_t mmsh_get_current_pos (mmsh_t *this) {
  ...
  calc_pos = this->asf_header_len + (this->chunk_seq_number + 1) * this->asf_packet_len;

  //lprintf ("> play rate: %.3f, pos: %lld, calc: %lld\n", this->mmsh_play_rate, this->current_pos, calc_pos);

  if (this->mmsh_play_rate == 1.0) {
    return this->current_pos;
  }
  else {
    return calc_pos;
