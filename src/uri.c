...
#define lprintf(...) fprintf(stderr, "mms-uri: " __VA_ARGS__)
#endif
 ...
 *
 **/
void gnet_uri_new_sub2 (const char* uri, const char** pi, GURI* guri)
{
  const char* temp;
  const char* p=*pi;
  temp = p;
...
int gnet_uri_new_sub1 (const char* uri, const char** pi, GURI* guri)
{
  const char* temp;
  const char* p=*pi;
  char *userinfo;
...
gnet_uri_new (const char* uri)
{
  ...
  if (*p == '/' && p[1] == '/')
  {
    if (gnet_uri_new_sub1(uri, &p, guri)<0)
      goto error;
  }

  /* Path (we are liberal and won't check if it starts with /) */
  gnet_uri_new_sub2(uri, &p, guri);
